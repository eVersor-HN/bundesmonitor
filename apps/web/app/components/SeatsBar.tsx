...
import { partyColorVar } from "@/lib/parteiFarben";
...
export function SeatsBar() {
  ...
  return (
    <Link href="/akteure/parteien" className="bm-card bm-card--hover bm-hud relative block">
      ...
      >
        {seated.map((p) => (
          <div
            key={p.name}
            style={{ width: `${(p.seats / total) * 100}%`, minWidth: "3px", background: partyColorVar(p.name) }}
        ...
        {seated.map((p) => (
          <li key={p.name} className="flex items-center gap-1.5 whitespace-nowrap text-[0.72rem]">
            <span
              ...
              style={{ background: partyColorVar(p.name) }}
            />
            <span>{p.name}</span>
