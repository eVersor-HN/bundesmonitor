...
export function MatterClient() {
  ...
  const sourceUrl = safeExternalHref(sourceId ? dipWebVorgangUrl(sourceId, matter.title) : null);
    ...
    <div className="flex flex-col gap-8">
      ...
      <section aria-labelledby="timeline-title" className="flex flex-col gap-4">
        ...
        {timeline.length === 0 ? (
          ...
          >
            {timeline.map((ev) => {
              const evHref = safeExternalHref(ev.source_url);
              return (
      ...
      <section aria-labelledby="docs-title" className="flex flex-col gap-4">
        ...
        {documents.length === 0 ? (
          ...
          <ul className="flex flex-col gap-2">
            {documents.map((doc, i) => {
              const docHref = safeExternalHref(doc.url);
              return (
              <li
                key={`${doc.document_number ?? doc.url ?? i}`}
                className="bm-card flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1"
              >
