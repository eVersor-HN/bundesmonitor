  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-col gap-2">
        {groups.map((g) => (
          <li key={g.slug}>
            <Link href={`/akteure/parteien?f=${g.slug}`} className="bm-card bm-card--tight bm-card--hover block">
              ...
              >
                <div
                  ...
                  style={{ width: `${Math.max(3, (g.count / max) * 100)}%`, background: partyColorVar(g.fraktion) }}
                />
              </div>
            </Link>
          </li>
