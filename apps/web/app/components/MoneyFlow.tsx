...
export function MoneyFlow() {
  ...
  const sourceHref = safeExternalHref(h.source.url);
  const detailHref = safeExternalHref(h.detailUrl);
  return (
    <section className="bm-card bm-card--roomy">
      ...
      <p className="mt-4 text-[0.68rem]" style={{ color: "var(--bm-text-muted)" }}>
        Quelle:{" "}
        {sourceHref ? (
          <a className="bm-link" href={sourceHref} rel="noopener noreferrer" target="_blank">
            {h.source.label}
          </a>
        ) : (
          h.source.label
        ...
        {detailHref ? (
          <a className="bm-link" href={detailHref} rel="noopener noreferrer" target="_blank">
            bundeshaushalt.de
          </a>
        ) : (
          "bundeshaushalt.de"
