...
export default function SettingsPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* -------- Projekt unterstützen (bewusst ganz oben) -------- */}
      ...
      <Group title="Datenzugang (DIP)">
        <p className="bm-field__hint" style={{ padding: "0.2rem 0 0.6rem" }}>
          Bundesmonitor holt die Daten direkt aus der amtlichen DIP-Schnittstelle des
          Bundestages – ohne eigenen Server.
        </p>

        {/* Öffentlicher Sammelschlüssel */}
        <div className="flex flex-col gap-2 py-3">
          <div className="flex flex-col gap-0.5">
            <span className="bm-field__label">Öffentlicher Sammelschlüssel{ownKey ? "" : " · aktiv"}</span>
            <span className="bm-field__hint">
              Standard, ohne Anmeldung – geteiltes Kontingent. Haltbarkeit ca. 6 Monate (rotiert
              automatisch).
            </span>
          </div>
          <button
            type="button"
            onClick={() => void loadPublicKey()}
            disabled={busy}
            className="bm-btn w-full justify-center"
          >
            Öffentlichen Schlüssel neu laden
          </button>
        </div>

        {/* Eigener Schlüssel */}
        <div className="flex flex-col gap-2 border-t py-3" style={{ borderColor: "var(--bm-border)" }}>
          <div className="flex flex-col gap-0.5">
            <label htmlFor="dipKey" className="bm-field__label">
              Eigener Schlüssel{ownKey ? " · aktiv" : " · optional"}
            </label>
            <span className="bm-field__hint">
              Schneller und unabhängig vom geteilten Kontingent. Kostenlos & personalisiert.
          ...
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => void validateKey()} disabled={busy} className="bm-btn w-full justify-center">
              Prüfen
            </button>
            <button type="button" onClick={save} className="bm-btn bm-btn--primary w-full justify-center">
              {t("opt.speichern")}
            </button>
          </div>
          {(conn || saved) && (
